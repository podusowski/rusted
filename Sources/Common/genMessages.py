...
def genMessageFactory():
	messagesHpp.write(
		...
		"{\n"
		"public:\n"
		"\tstatic boost::shared_ptr<AbstractMessage> create(::Common::RustedCodec::IReadBuffer & buf)\n\t{\n"
...
class CppOutput:
    ...
    def __write_id_enum(self):
        ...
        for message in self.messages:
            ...
            self.f.write("\t" + message.id + " = " + str(id) + ",\n")
            id = id + 1
...
class CppMessageStruct:
    ...
    def generate(self):
        ...
        if len(self.message.params) > 0:
            ...
            s = s + self.__generate_empty_serialize_method()
            s = s + self.__generate_empty_to_string_method()
    ...
    def __generate_empty_serialize_method(self):
        s = (
            "\tvoid serialize(Common::RustedCodec::IWriteBuffer &) const\n"
            "\t{\n"
            "\t\t// do nothing\n"
            "\t}\n"
            "\n"
    ...
    def __generate_empty_to_string_method(self):
        s = (
            "\tstd::string toString() const\n"
            "\t{\n"
            "\t\treturn std::string();\n"
...
for message_xml_node in messages.childNodes[0].getElementsByTagName("message"):
    ...
    msgs.append(message)
hpp = CppOutput(msgs, "Messages/Messages.v2.hpp")
hpp.generate()
